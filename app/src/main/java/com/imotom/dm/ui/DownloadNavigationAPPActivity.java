...
import com.imotom.dm.service.DownloadDaoHangAPPService;
...
public class DownloadNavigationAPPActivity extends AppCompatActivity implements View.OnClickListener, Consts {

    @BindView(R.id.DaoHangAPP_pb)
    ProgressBar progress1;
    @BindView(R.id.DaoHangAPP_btn_down)
    Button btn;
    /**
     * 广播接收者
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dao_hang_di_zhi_tui_song_appdownload);
    ...
    private void init() {
        // 激活Service
        Intent intent = new Intent(this, DownloadDaoHangAPPService.class);
        startService(intent);
 ...
 */
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.DaoHangAPP_btn_down:
                 ...
                 */
                sendBroadcast(new Intent().setAction(ACTION_DOWNLOAD_OR_PAUSE).putExtra(EXTRA_DOWNLOAD_GUJIAN_NAME_TEXT, URL_downloadNavAPPUrl));
    ...
    private class InnerReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_UPDATE_PROGRESS.equals(action)) {
                ...
                Toast.makeText(DownloadNavigationAPPActivity.this, "下载成功", Toast.LENGTH_SHORT).show();

                /*// 取消注册广播接收者
                unregisterReceiver(receiver);*/
                // 停止Service
                Intent intentStop = new Intent(DownloadNavigationAPPActivity.this, DownloadDaoHangAPPService.class);
                stopService(intentStop);
                btn.setText(String.valueOf("开始下载"));
                ...
                //install(Environment.getExternalStorageDirectory().getPath() + "/imotom/" + "zxc.com.gkdvr_21.apk");
                //自动安装apk
                installAPK(Environment.getExternalStorageDirectory().getPath() + "/imotom/" + NAVIGATION_APP_NAME, context);

                //接收 正在下载状态的广播 后的逻辑
            } else if (ACTION_SET_DOWNLOAD_STATE.equals(action)) {
                btn.setVisibility(View.VISIBLE);
                progress1.setVisibility(View.VISIBLE);
                Toast.makeText(context, "下载固件中", Toast.LENGTH_SHORT).show();
                btn.setText("取消下载");
                isDownload = true;
                //接收 取消下载状态的广播 后的逻辑
    ...
    private void installAPK(String path, Context context) {
        ...
        if (Build.VERSION.SDK_INT < 23) {
            ...
            File file = new File(Environment.getExternalStorageDirectory().getPath() + "/imotom", NAVIGATION_APP_NAME);
            if (file.exists()) {
